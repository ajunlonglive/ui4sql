...
public class ElementPlugin extends AbsDivisionPlugin {
	...
	public ElementPlugin() throws services.ServicesException {
		...
		this.setListHeaders( new String[] { "Seq", "Title", "Version", "Status",
				getParentTarget() });

		this.setMoreListColumns(new  String[] { "seq_no", "telement.title_nm",
				"telement.version_id", "s.code_desc as status_desc",
				parentTableName + ".title_nm as thing_name" });

		this.setMoreListJoins(new  String[] {
				" left join tcodes s on telement.status_cd = s.code_value and s.code_type_id  = 60 ",
				" left join tproject on " + parentTableName
						+ ".project_id = tproject.project_id ",
				"  join " + parentTableName + " on telement.parent_id = "
	...
	public WebField getListSelector(int columnNumber) {
		...
		debug("getting list selector");
		String sQuery = "select " + parentKeyName + "," + parentKeyName
				+ ", title_nm   from " + parentTableName + " where "
				+ parentTableName + "." + grandParentKeyName + " = "
				+ sm.getApplicationId().toString();

		return new WebFieldSelect(filterName,
				sm.Parm(filterName).length() == 0 ? new Integer("0")
						: new Integer(sm.Parm(filterName)), db, sQuery,
	...
	public boolean beforeAdd(Hashtable ht) {
		...
		ht.put("parent_kind_cd", new DbFieldString("parent_kind_cd",
				elementKindCd));
	...
	public Hashtable getWebFields(String parmMode)
		...
		ht.put("example_tx", new WebFieldString("example_tx", db
				.getText("example_tx"), 64, 255));

		ht.put("format_tx", new WebFieldString("format_tx", db
				.getText("format_tx"), 64, 255));

		ht.put("values_tx", new WebFieldString("values_tx", db
				.getText("values_tx"), 64, 255));
		...
		ht.put("version_id", new WebFieldString("version_id", (addMode ? "0"
				: db.getText("version_id")), 4, 4));

		ht.put("seq_no", new WebFieldString("seq_no", db.getText("seq_no"), 4,
				4));

		ht.put("length_no", new WebFieldString("length_no", (addMode ? "0" : db
				.getText("length_no")), 4, 4));
		...
		ht.put("type_cd", new WebFieldSelect("type_cd", addMode ? "" : db
				.getText("type_cd"), sm.getCodes("DATATYPES")));
